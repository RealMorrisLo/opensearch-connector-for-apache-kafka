...
import static io.aiven.kafka.connect.opensearch.OpensearchSinkConnectorConfig.*;
...
public class BulkProcessorTest {
  ...
  public void flushing(final @Mock RestHighLevelClient client) throws IOException {
    ...
    clientAnswer.expect(
        List.of(newIndexRequest(1), newIndexRequest(2), newIndexRequest(3)), successResponse());
  ...
  public void addBlocksWhenBufferFull(final @Mock RestHighLevelClient client) {
    ...
    assertThrows(
        ConnectException.class, () -> bulkProcessor.add(newIndexRequest(43), addTimeoutMs));
  }
  ...
  @Test
  public void retryableErrorsHitMaxRetries(final @Mock RestHighLevelClient client)
      throws IOException {
    final var clientAnswer = new ClientAnswer();
    when(client.bulk(any(BulkRequest.class), eq(RequestOptions.DEFAULT))).thenAnswer(clientAnswer);
  ...
  public void failOnMalformedDoc(final @Mock RestHighLevelClient client) throws IOException {
    ...
    final var bulkProcessor = new BulkProcessor(Time.SYSTEM, client, config);
    clientAnswer.expect(
        List.of(newIndexRequest(42), newIndexRequest(43)), failedResponse(errorInfo));
  ...
  @Test
  public void ignoreOrWarnOnMalformedDoc(final @Mock RestHighLevelClient client)
      throws IOException {
    final var clientAnswer = new ClientAnswer();
    when(client.bulk(any(BulkRequest.class), eq(RequestOptions.DEFAULT))).thenAnswer(clientAnswer);
    ...
    final List<BehaviorOnMalformedDoc> behaviorsToTest =
        List.of(BehaviorOnMalformedDoc.WARN, BehaviorOnMalformedDoc.IGNORE);
