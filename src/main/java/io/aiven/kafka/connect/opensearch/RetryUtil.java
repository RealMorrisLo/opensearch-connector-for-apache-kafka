 ...
 * becomes overwhelmed. Adding jitter attempts to prevent a thundering herd, where large numbers of
 * requests from many tasks overwhelm the ES service, and without randomization all tasks retry at
 * the same time. Randomization should spread the retries out and should reduce the overall time
 * required to complete all attempts. See <a
 * href="https://www.awsarchitectureblog.com/2015/03/backoff.html">this blog post</a> for details.
 */
public class RetryUtil {
   ...
   * normal exponential backoff as {@code initialRetryBackoffMs << retryAttempt}, and then chooses a
   * random value between 0 and that value.
   *
   * @param retryAttempts the number of previous retry attempts; must be non-negative
   * @param initialRetryBackoffMs the initial time to wait before retrying; assumed to be 0 if value
   *     is negative
   ...
   */
  public static long computeRandomRetryWaitTimeInMillis(
      final int retryAttempts, final long initialRetryBackoffMs) {
   ...
   * exponential backoff as {@code initialRetryBackoffMs << retryAttempt}, bounded to always be less
   * than {@link #MAX_RETRY_TIME_MS}.
   *
   * @param retryAttempts the number of previous retry attempts; must be non-negative
   * @param initialRetryBackoffMs the initial time to wait before retrying; assumed to be 0 if value
   *     is negative
   ...
   *     {@code initialRetryBackoffMs} is negative
   */
  public static long computeRetryWaitTimeInMillis(
      final int retryAttempts, final long initialRetryBackoffMs) {
    if (initialRetryBackoffMs < 0) {
      return 0;
    }
      ...
      try {
        ...
        if (attempts < maxAttempts && e.getClass().equals(repeatableException)) {
          final long sleepTimeMs =
              computeRandomRetryWaitTimeInMillis(retryAttempts, retryBackoffMs);
          final var msg =
              String.format(
          ...
          final var msg =
              String.format("Failed to %s after total of %s attempt(s)", callName, attempts);
          LOGGER.error(msg, e);
