...
public class RecordConverter {
    ...
    if (keySchema == null) {
      ...
      if (schemaType == null) {
        throw new DataException(
            String.format(
                "Java class %s does not have corresponding schema type.", key.getClass()));
      }
    } else {
  ...
  public DocWriteRequest<?> convert(final SinkRecord record, final String index) {
    ...
    final var id =
        (config.ignoreKeyFor(record.topic()))
  ...
  private String getPayload(final SinkRecord record) {
    ...
    final Schema schema =
        config.ignoreSchemaFor(record.topic())
            ? record.valueSchema()
  ...
  private Schema preProcessMapSchema(final Schema schema) {
    ...
    final String valueName =
        valueSchema.name() == null ? valueSchema.type().name() : valueSchema.name();
    ...
    if (config.useCompactMapEntries() && keySchema.type() == Schema.Type.STRING) {
      final SchemaBuilder result =
          SchemaBuilder.map(preprocessedKeySchema, preprocessedValueSchema);
  ...
  private Schema preProcessStructSchema(final Schema schema) {
    final SchemaBuilder builder =
        copySchemaBasics(schema, SchemaBuilder.struct().name(schema.name()));
  ...
  private Object preProcessArrayValue(
      final Object value, final Schema schema, final Schema newSchema) {
  ...
  private Object preProcessMapValue(
      final Object value, final Schema schema, final Schema newSchema) {
    final Schema keySchema = schema.keySchema();
    final Schema valueSchema = schema.valueSchema();
    ...
    for (final Map.Entry<?, ?> entry : map.entrySet()) {
      ...
      mapStruct.put(
          Mapping.VALUE_FIELD, preProcessValue(entry.getValue(), valueSchema, mapValueSchema));
      mapStructs.add(mapStruct);
    }
    return mapStructs;
  }

  private Object preProcessStructValue(
      final Object value, final Schema schema, final Schema newSchema) {
