...
public class OpensearchClient implements AutoCloseable {
  ...
  protected OpensearchClient(
      final RestHighLevelClient client, final OpensearchSinkConnectorConfig config) {
    this.client = client;
    this.config = config;
  ...
  public boolean hasMapping(final String index) {
    ...
    final var response =
        withRetry("", () -> client.indices().getMapping(request, RequestOptions.DEFAULT));
  ...
  private static class HttpClientConfigCallback
        ...
        for (final var httpHost : config.httpHosts()) {
          credentialsProvider.setCredentials(
              ...
              new UsernamePasswordCredentials(
                  config.connectionUsername(), config.connectionPassword().value()));
        }
    ...
    private PoolingNHttpClientConnectionManager createConnectionManager() {
      try {
        final var ioReactorConfig =
            IOReactorConfig.custom()
                .setConnectTimeout(config.connectionTimeoutMs())
                .setSoTimeout(config.readTimeoutMs())
                .build();

        final var sslStrategy =
